...
class BuscaminasExperto:
    ...
    def pedir_dato(self, r, c, motivo):
        ...
        try:
            ...
            if min_val == max_val:
                # Si solo hay una opción lógica, informamos y rellenamos automáticamente.
                num = min_val
                messagebox.showinfo("Valor único", f"En ({r}, {c}) solo se puede ingresar {num}.")
            else:
                ...
                num = simpledialog.askinteger(
                    ...
                    f"Motivo: {motivo}\n\n"
                    f"Casilla: ({r}, {c})\n"
                    f"Valores lógicos posibles: {valores_posibles}\n"
                    f"(Rango {min_val} a {max_val})\n\n"
                    f"¿Cuántas minas ves alrededor?"
                )

            # Si el usuario cierra el cuadro en la "X", continuar sin cerrar el juego.
            if num is None:
                self.restaurar_estilo_casilla(r, c)
                return
